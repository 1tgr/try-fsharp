...
function mtFetchUser(cb) {
    ...
    if ( ( cb == 'mtSetUser' ) && mtGetUser() ) {
        ...
        script.src = 'http://mt.partario.com/mt-comments.cgi?__mode=session_js&blog_id=9&jsonp=' + cb + '&ts=' + ts;
...
function mtCommentOnSubmit(f) {
    if (!mtRequestSubmitted) {
        ...
        if (f.armor)
            f.armor.value = '6caead80cac448c04422935959824ead87b3c59b';
...
function mtSignIn() {
    ...
    var url = 'http://mt.partario.com/mt-comments.cgi?__mode=login&blog_id=9';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
        } else {
            url += '&return_url=http%3A%2F%2Ftryfs.net%2F';
        }
    } else {
